...
**怎么收拾的**：改成 handler 自身的 `self.reply_text(text, msg)`（POST 消息自带 `session_webhook`，官方推荐用法），并写了一条**模拟真实钉钉消息走 process 全链路**的回归测试（`test_im_process.py`），保证这条路径永远被 CI 覆盖。

## 4. 我怎么验证 AI 生成的代码是对的？

四层验证，从快到慢：

1. **单元测试（Mock LLM，不依赖真实 API）**：`tests/` 里 45 条用例，用 FakeClient 替换 OpenAI client，覆盖知识库增删改、同名替换、拒答判定、工具调用循环、流式输出、钉钉 AI 卡片流式回复、飞书 AI 卡片流式回复、企业微信回调、引用定位、错误重试与降级、文件上传问答、可观测性、会话隔离。本地 `uv run pytest tests/ -v` 全绿（实测 45 passed）。
