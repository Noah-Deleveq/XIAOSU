...
1. **单元测试（Mock LLM，不依赖真实 API）**：`tests/` 里 41 条用例，用 FakeClient 替换 OpenAI client，覆盖知识库增删改、同名替换、拒答判定、工具调用循环、流式输出、钉钉 AI 卡片流式回复、引用定位、错误重试与降级、文件上传问答、可观测性、会话隔离。本地 `uv run pytest tests/ -v` 全绿（实测 41 passed）。
